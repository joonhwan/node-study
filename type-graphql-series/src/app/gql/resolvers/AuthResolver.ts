...
import { UserCreateInputType } from "./inputs/UserCreateInputType";
...
import { ChangePasswordInput } from "@/app/gql/resolvers/inputs/ChangePasswordInputType";
...
export class AuthResolver {
  ...
  @Mutation(() => User)
  async register(@Arg("data") data: UserCreateInputType): Promise<User> {
  ...
  @Mutation(() => Boolean)
  async confirm(@Arg("confirmId") confirmId: string) {
    const confirmation = await UserConfirmation.findOne({
      where: { confirmId },
    });
    if (!confirmation) {
      return false;
  ...
  async resetPassword(@Arg("email") email: string) {
    ...
    await sendEmail(
      email,
      `http://localhost:3000/user/reset-password/${reset.token}`
    );
    return true;
  }
